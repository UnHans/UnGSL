...
from opengsl.module.model.slaps import SLAPS,SparseUnGSL
import torch
import time
...
class SLAPSSolver(Solver):
    ...
    def learn(self, debug=False,run_time=None):
        ...
        for epoch in range(1, self.conf.training['n_epochs'] + 1):
            ...
            output, loss_dae, adj = self.model(self.feats,self.UnGSL)
    ...
    def evaluate(self, test_mask):
        ...
        self.model.eval()
        with torch.no_grad():
            output, _, _ = self.model(self.feats,self.UnGSL)
            # if len(test_mask)==5666:
            #     softmax_func=torch.nn.Softmax(dim=1)
            #     prob_matrix=softmax_func(output)
            #     prob_matrix=torch.clamp(prob_matrix, min=1e-6)
            #     uncertrainy = -torch.sum(prob_matrix * torch.log(prob_matrix), dim=1)
            #     torch.save(uncertrainy,"SLAPSRomanEntropy"+str(self.run_time)+".pt")
    ...
    def set_method(self):
        ...
        '''
        self.UnGSL=SparseUnGSL(self.dataset,self.conf).to(self.device)
