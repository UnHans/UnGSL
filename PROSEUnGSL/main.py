...
class Experiment:
    ...
    def node_anchor_loss_cls_auc(self, classifier, ori_adj, features, node_anchor_adj, mask, labels, fusion_ratio = 0.3,epochs=None,trial=None):
        ...
        #print(epochs,trial)
        # if torch.sum(mask)==1000 and ( (epochs == 146 and trial==0) or (epochs == 237 and trial==1) or (epochs == 208 and trial==2)  or (epochs == 137 and trial==3) or (epochs == 173 and trial==4)):
        #     softmax_func=torch.nn.Softmax(dim=1)
        #     prob_matrix=softmax_func(logits)
        #     uncertrainy = -torch.sum(prob_matrix * torch.log(prob_matrix), dim=1)
        #     torch.save(uncertrainy,"PROSEPubmedEntropy"+str(trial)+".pt")
    ...
    def train(self, args):
        ...
        #print(torch.nonzero(test_mask)[:100])#169 183
        test_accuracies = []
        ...
        for trial in range(args.ntrials):
            ...
            ungsl=AnchorUnGSL(args.dataset,args.init_value,args.beta)
            ...
            optimizer_ungsl=torch.optim.Adam(ungsl.parameters(),lr=args.ungsl_lr)
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                node_anchor_adj=ungsl(node_anchor_adj,anchor_nodes_idx)
                ...
                optimizer_ungsl.zero_grad()
                final_loss.backward()

                optimizer_cl.step()
                optimizer_ungsl.step()
                ...
                if epoch % args.eval_freq == 0:
                    ...
                    test_loss, test_accu, test_f1_macro, test_f1_micro, test_auc = self.node_anchor_loss_cls_auc(classifier, anchor_adj, features, node_anchor_adj, test_mask, labels, args.emb_fusion_ratio,epochs=epoch,trial=trial)
                    nni.report_intermediate_result(val_accu.item())
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ntrials', type=int, default=9)
    parser.add_argument('-init_value', type=float, default=0.01)
    parser.add_argument('-beta', type=float, default=0.01)
    parser.add_argument('-ungsl_lr', type=float, default=0.001)
    parser.add_argument('-seeds', nargs='+', type=list, default=[0,1,2,3,4,6,7,8,9])
    ...
    print(params)
    args.lr_cl=params["lr_cl"]
    args.lr_gsl=params["lr_gsl"]
    args.init_value=params["init_value"]
    args.ungsl_lr=params["ungsl_lr"]
    args.beta=params["beta"]
    args.epsilon=params["epsilon"]
    args.maskfeat_rate_learner=params["maskfeat_rate_learner"]
    args.maskfeat_rate_anchor=params["maskfeat_rate_anchor"]
    args.dropedge_rate=params["dropedge_rate"]
    args.mi_ratio=params["mi_ratio"]
    print(args)
    ...
    test_acc=experiment.train(args)
    print(test_acc)
