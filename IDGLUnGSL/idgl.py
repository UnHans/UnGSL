...
class UnGSL(nn.Module):
    def __init__(self,conf=None):
        ...
        num_nodes=0
        device="cuda:0"
        """Load node entropy vector and transform it to confidence vector"""
        if conf.dataset["name"]=="cora":
            Entropy=torch.load("/home/hs/OpenGSL/"+"IDGLCoraEntropy"+".pt",map_location=device)
            num_nodes=Entropy.size()[0]
        elif conf.dataset["name"]=="citeseer":
            Entropy=torch.load("/home/hs/OpenGSL/"+"IDGLCiteseerEntropy"+".pt",map_location=device)
            num_nodes=Entropy.size()[0]
        elif conf.dataset["name"]=="roman-empire":
            Entropy=torch.load("/home/hs/OpenGSL/"+"IDGLRomanEntropy"+".pt",map_location=device)
            num_nodes=Entropy.size()[0]
        elif conf.dataset["name"]=="pubmed":
            Entropy=torch.load("/home/hs/OpenGSL/"+"IDGLPubmedEntropy"+".pt",map_location=device)
            num_nodes=Entropy.size()[0]
        elif conf.dataset["name"]=="arxiv":
            Entropy=torch.load("/home/hs/NOpenGSL/"+"IDGLArxivEntropy"+".pt",map_location=device)
            num_nodes=Entropy.size()[0]
        ...
        self.confidence_vector=confidence_vector
    ...
    def forward(self,learned_adj):
        ...
        learned_adj=torch.sparse_coo_tensor(indices, new_values,learned_adj.shape)
        return learned_adj.to_dense()
        ...
        #     confidence_matrix = self.confidence_vector[anchor_node_idx].unsqueeze(0).repeat(self.confidence_vector.size(0), 1)
        # confidence_matrix=confidence_matrix * ( (learned_adj>=0).int())
        # # print(confidence_matrix[:100])
